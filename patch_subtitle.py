...
parser.add_option('--bitrate',
                  ...
                  help="ffmpeg vcodec -b:v", default="6000k")
...
for inputvideo in inputvideos:
    ...
    ffcmd = 'ffmpeg -y -hide_banner -loglevel error -stats -vn -i "' + inputvideo + '"' + \
        " -acodec copy " + '"' + inputvideo + ".tmp.aac" + '"'
    print(ffcmd)
    subprocess.call(ffcmd, shell=True, env=env)
    ffcmd = "ffmpeg -y -hide_banner -loglevel error -stats -i " + '"' + out_name + '"' + \
        " -i " + '"' + inputvideo + '.tmp.aac" -vcodec ' + \
        ...
        options.audio_bitrate + " -ar 44100 -f flv " + \
            '"' + inputvideo + ".autosubed.flv" + '"'
    print(ffcmd)
    subprocess.call(ffcmd, shell=True, env=env)
